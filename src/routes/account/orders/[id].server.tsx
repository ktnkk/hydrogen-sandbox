...
const OrderDetails = ({ response }: HydrogenRouteProps) => {
  ...
  return (
    <Layout>
      ...
      <div className='p-6 w-full sm:grid-cols-1 md:p-8 lg:p-12 lg:py-6'>
        <div>
          ...
          <div className='grid gap-12 items-start sm:grid-cols-1 sm:divide-y sm:divide-gray-200 md:grid-cols-4 md:gap-16'>
            <table className='my-8 min-w-full divide-y divide-gray-300 md:col-span-3'>
              <thead>
                <tr className='align-baseline '>
                  <th
                    scope='col'
                    className='pr-3 pb-4 pl-0 font-semibold text-left'
                  >
                    Product
                  </th>
                  <th
              ...
              <tbody className='divide-y divide-gray-200'>
                {lineItems.map((lineItem) => (
                  <tr key={lineItem.variant!.id}>
                    <td className='py-4 pr-3 pl-0 w-full max-w-0 align-top sm:w-auto sm:max-w-none sm:align-middle'>
                      <div className='flex gap-6'>
                        <Link
                          to={`/products/${lineItem.variant!.product!.handle}`}
                        >
                          {lineItem?.variant?.image && (
                            <div className='aspect-square w-24 card-image'>
                              <Image
                    ...
                    <td className='hidden py-4 px-3 text-right align-top sm:table-cell sm:align-middle'>
                      <Money data={lineItem.variant!.priceV2!} />
                    </td>
                    <td className='hidden py-4 px-3 text-right align-top sm:table-cell sm:align-middle'>
                      {lineItem.quantity}
                    </td>
                    <td className='py-4 px-3 text-right align-top sm:table-cell sm:align-middle'>
                      <Text>
              ...
              <tfoot>
                {((discountValue && discountValue.amount) ||
                  ...
                  <tr>
                    <th
                      scope='row'
                      colSpan={3}
                      className='hidden pt-6 pr-3 pl-6 font-normal text-right sm:table-cell md:pl-0'
                    >
                      <Text>Discounts</Text>
                    </th>
                    <th
                    ...
                    <td className='pt-6 pr-4 pl-3 font-medium text-right text-green-700 md:pr-3'>
                      {discountPercentage ? (
                ...
                <tr>
                  <th
                    scope='row'
                    colSpan={3}
                    className='hidden pt-4 pr-3 pl-6 font-normal text-right sm:table-cell md:pl-0'
                  >
                    Tax
                  </th>
                  <th
                ...
                <tr>
                  <th
                    scope='row'
                    colSpan={3}
                    className='hidden pt-4 pr-3 pl-6 font-semibold text-right sm:table-cell md:pl-0'
                  >
                    Total
                  </th>
                  <th
                  ...
                  <td className='pt-4 pr-4 pl-3 font-semibold text-right md:pr-3'>
