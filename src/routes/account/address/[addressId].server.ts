...
const deleteAddress = async (
  ...
  const { data, errors } = await queryShop<{
    ...
    variables: {
      customerAccessToken,
      id: decodeURIComponent(params.addressId as string),
    },
...
const updateAddress = async (
  ...
  if (isDefaultAddress) {
    const { data, errors } = await setDefaultAddress(
      queryShop,
      decodeURIComponent(params.addressId as string),
      customerAccessToken,
    );
...
export const api = async (
  ...
  if (request.method === 'DELETE')
    return deleteAddress(customerAccessToken as string, params, queryShop);
